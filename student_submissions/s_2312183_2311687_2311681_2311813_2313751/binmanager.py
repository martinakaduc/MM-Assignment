...
Algorithm = maximal_rectangles.MaximalRectangle

class BinManager(Policy):
    """
    Interface Class.
    """
    def __init__(self, bin_width: int = 8,
                 ...
                 unused_bins: List =[]) -> None:
        #fixed
        self.unused_bins = unused_bins
    ...
    def _bin_factory(self,item) -> Any:
        """
        ...
        bin = None
        for i in range(len(self.unused_bins)):
            if (self.unused_bins[i][1][0] >= item.width and self.unused_bins[i][1][1] >= item.height) or (self.unused_bins[i][1][0] >= item.height and self.unused_bins[i][1][1] >= item.width):
                bin = self.unused_bins.pop(i)
                break
            
        # width, height = self._get_stock_size_(bin)
        if(bin == None):
            raise ValueError("Error! item too big for bin")


        width, height = bin[1]
        return maximal_rectangles.MaximalRectangle(width, height, self.rotation, self.heuristic,bin[0])
    ...
    def _bin_first_fit(self, item: item.Item) -> None:
        ...
        for binn in self.bins:
            ...
            if result:
                break
        if not result :
            self.bins.append(self._bin_factory(item=item))
    ...
    def _bin_best_fit(self, item: item.Item) -> bool:
        ...
        # print(len(self.unused_bins))
        new_bin = self._bin_factory(item=item)
